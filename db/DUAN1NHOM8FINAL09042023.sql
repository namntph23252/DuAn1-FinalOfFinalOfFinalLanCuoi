create database NHOM8_SACH_FINAL3
use NHOM8_SACH_FINAL3


-- CHAY LAN 1
-- auto ma sach
CREATE FUNCTION AUTO_MASACH()
RETURNS VARCHAR(10)
AS
BEGIN
	DECLARE @ID VARCHAR(5)
	IF (SELECT COUNT(ma) FROM sach) = 0
		SET @ID = '0'
	ELSE
		SELECT @ID = MAX(RIGHT(ma, 3)) FROM sach
		SELECT @ID = CASE
			WHEN @ID >= 0 and @ID < 9 THEN 'SA00' + CONVERT(CHAR, CONVERT(INT, @ID) + 1)
			WHEN @ID >= 9 THEN 'SA0' + CONVERT(CHAR, CONVERT(INT, @ID) + 1)
		END
	RETURN @ID
END

-- CHAY LAN 2
IF OBJECT_ID('SACH') IS NOT NULL
DROP TABLE SACH
GO
CREATE TABLE SACH(
	ID int Identity not null PRIMARY KEY,
	Ma VARCHAR(20) UNIQUE DEFAULT DBO.AUTO_MASACH() ,
	TENSACH nvarchar(80),
	IdNXB INT,
	MOTA NVARCHAR(50) DEFAULT NULL,
	GIANHAP DECIMAL(20,0) DEFAULT 0,
	GIABAN DECIMAL(20,0) DEFAULT 0,
	TRANGTHAI BIT DEFAULT 0,
	IMAGELINK nvarchar(80),
	MAVACH varchar(50)
	)

-- CHAY LAN 3
-- FUNCTION AUTO MATG
CREATE FUNCTION AUTO_MATG()
RETURNS VARCHAR(10)
AS
BEGIN
	DECLARE @ID VARCHAR(5)
	IF (SELECT COUNT(ma) FROM TACGIA) = 0
		SET @ID = '0'
	ELSE
		SELECT @ID = MAX(RIGHT(ma, 3)) FROM TACGIA
		SELECT @ID = CASE
			WHEN @ID >= 0 and @ID < 9 THEN 'TG00' + CONVERT(CHAR, CONVERT(INT, @ID) + 1)
			WHEN @ID >= 9 THEN 'TG0' + CONVERT(CHAR, CONVERT(INT, @ID) + 1)
		END
	RETURN @ID
END
-- CHAY LAN 4
CREATE TABLE TACGIA (
ID int identity PRIMARY KEY ,
Ma VARCHAR(20) UNIQUE DEFAULT DBO.AUTO_MATG() ,
TEN NVARCHAR(50) DEFAULT NULL,
TRANGTHAI INT DEFAULT 0
)
-- CHAY LAN 5
IF OBJECT_ID('SACH_TACGIA') IS NOT NULL
DROP TABLE SACH_TACGIA
GO
CREATE TABLE SACH_TACGIA
(
ID_TG int NOT NULL,
ID_SACH int NOT NULL,
CONSTRAINT FK_SACHTG_SACH FOREIGN KEY(ID_SACH) REFERENCES SACH,
CONSTRAINT FK_SACHTG_TACGIA FOREIGN KEY(ID_TG) REFERENCES TACGIA,
CONSTRAINT PK_SACHTG PRIMARY KEY(ID_SACH,ID_TG)
)
-- CHAY LAN 6
CREATE FUNCTION AUTO_MATL()
RETURNS VARCHAR(5)
AS
BEGIN
	DECLARE @ID VARCHAR(5)
	IF (SELECT COUNT(MA) FROM THELOAI) = 0
		SET @ID = '0'
	ELSE
		SELECT @ID = MAX(RIGHT(MA, 3)) FROM THELOAI
		SELECT @ID = CASE
			WHEN @ID >= 0 and @ID < 9 THEN 'TL00' + CONVERT(CHAR, CONVERT(INT, @ID) + 1)
			WHEN @ID >= 9 THEN 'TL0' + CONVERT(CHAR, CONVERT(INT, @ID) + 1)
		END
	RETURN @ID
END
-- CHAY LAN 7

CREATE TABLE THELOAI(
ID int identity PRIMARY KEY ,
MA VARCHAR(20) UNIQUE  DEFAULT DBO.AUTO_MATL() ,
TEN NVARCHAR(80) DEFAULT NULL,
TRANGTHAI INT DEFAULT 0
)

-- CHAY LAN 8
IF OBJECT_ID('SACH_THELOAI') IS NOT NULL
DROP TABLE SACH_THELOAI
GO
CREATE TABLE SACH_THELOAI(
	ID_SACH int not null,
	ID_TL int not null,
	CONSTRAINT PK_SACHTL PRIMARY KEY(ID_sach,ID_TL) ,
	CONSTRAINT FK_SachTL_SACH FOREIGN KEY(ID_sach) REFERENCES SACH(ID),
	CONSTRAINT FK_SACHTL_Theloai FOREIGN KEY(ID_TL) REFERENCES THELOAI(ID)
	)
-- CHAY LAN 9
CREATE FUNCTION AUTO_MANXB()
RETURNS VARCHAR(5)
AS
BEGIN
	DECLARE @ID VARCHAR(5)
	IF (SELECT COUNT(MA) FROM NXB) = 0
		SET @ID = '0'
	ELSE
		SELECT @ID = MAX(RIGHT(MA, 3)) FROM NXB
		SELECT @ID = CASE
			WHEN @ID >= 0 and @ID < 9 THEN 'NX00' + CONVERT(CHAR, CONVERT(INT, @ID) + 1)
			WHEN @ID >= 9 THEN 'NX0' + CONVERT(CHAR, CONVERT(INT, @ID) + 1)
		END
	RETURN @ID
END
-- CHAY LAN 10
CREATE TABLE NXB(
ID int identity PRIMARY KEY ,
Ma VARCHAR(20) UNIQUE  DEFAULT DBO.AUTO_MANXB() ,
TEN NVARCHAR(50) DEFAULT NULL,
TRANGTHAI INT DEFAULT 0
)
-- CHAY LAN 11
CREATE FUNCTION AUTO_MAND()
RETURNS VARCHAR(5)
AS
BEGIN
	DECLARE @ID VARCHAR(5)
	IF (SELECT COUNT(MA) FROM NguoiDung) = 0
		SET @ID = '0'
	ELSE
		SELECT @ID = MAX(RIGHT(MA, 3)) FROM NguoiDung
		SELECT @ID = CASE
			WHEN @ID >= 0 and @ID < 9 THEN 'ND00' + CONVERT(CHAR, CONVERT(INT, @ID) + 1)
			WHEN @ID >= 9 THEN 'ND0' + CONVERT(CHAR, CONVERT(INT, @ID) + 1)
		END
	RETURN @ID
END
-- CHAYLAN 12
IF OBJECT_ID('NGUOIDUNG') IS NOT NULL
DROP TABLE NGUOIDUNG 
GO
CREATE TABLE NGUOIDUNG(
ID int identity PRIMARY KEY ,
MA VARCHAR(20) UNIQUE DEFAULT DBO.AUTO_MAND(),
HOTEN NVARCHAR(50) DEFAULT NULL,
EMAIL NVARCHAR(30) DEFAULT NULL,
GIOITINH BIT DEFAULT 0,
NGAYSINH DATE DEFAULT NULL,
DIACHI NVARCHAR(100) DEFAULT NULL,
SDT VARCHAR(30) DEFAULT NULL,
MATKHAU VARCHAR(MAX) DEFAULT NULL,
CHUCVU Nvarchar(10),
ImageLINK nvarchar(100) null,
TRANGTHAI bit DEFAULT 0
)
-- CHAY LAN 13
CREATE FUNCTION AUTO_MAKH()
RETURNS VARCHAR(5)
AS
BEGIN
	DECLARE @ID VARCHAR(5)
	IF (SELECT COUNT(MAKH) FROM KHACHHANG) = 0
		SET @ID = '0'
	ELSE
		SELECT @ID = MAX(RIGHT(MAKH, 3)) FROM KHACHHANG
		SELECT @ID = CASE
			WHEN @ID >= 0 and @ID < 9 THEN 'KH00' + CONVERT(CHAR, CONVERT(INT, @ID) + 1)
			WHEN @ID >= 9 THEN 'KH0' + CONVERT(CHAR, CONVERT(INT, @ID) + 1)
		END
	RETURN @ID
END
-- CHAY LAN 14
CREATE TABLE KHACHHANG(
	ID int identity PRIMARY KEY ,
	MAKH VARCHAR(5) UNIQUE DEFAULT DBO.AUTO_MAKH(),
	HOTEN NVARCHAR(50) NOT NULL,
	SDT VARCHAR(11) NULL,
	EMAIL VARCHAR(50) NULL,
	TRANGTHAI bit DEFAULT 0
)
insert into khachhang(hoten, sdt) values(N'Khách Lẻ', 0)
-- CHAY LAN 15 
CREATE FUNCTION AUTO_MAHD()
RETURNS VARCHAR(5)
AS
BEGIN
	DECLARE @ID VARCHAR(5)
	IF (SELECT COUNT(MA) FROM HOADON) = 0
		SET @ID = '0'
	ELSE
		SELECT @ID = MAX(RIGHT(MA, 3)) FROM HOADON
		SELECT @ID = CASE
			WHEN @ID >= 0 and @ID < 9 THEN 'HD' + CONVERT(CHAR, CONVERT(INT, @ID) + 1)
			WHEN @ID >= 9 THEN 'HD' + CONVERT(CHAR, CONVERT(INT, @ID) + 1)
		END
	RETURN @ID
END
-- CHAY LAN 16
IF OBJECT_ID('HOADON') IS NOT NULL
DROP TABLE HOADON
GO
CREATE TABLE HOADON(
ID INT IDENTITY PRIMARY KEY ,
IDKH INT,
IDND INT,
MA VARCHAR(20) UNIQUE DEFAULT DBO.AUTO_MAHD(),
TONGTIEN decimal(10,2),
IDKHUYENMAI INT,
TONGTIENSAUKHUYENMAI DECIMAL(10,2),
NgayTao DATE DEFAULT NULL,
TrangThai BIT DEFAULT 0,
)
-- CHAY LAN 17
IF OBJECT_ID('HOADONCT') IS NOT NULL
DROP TABLE HOADONCT 
GO
CREATE TABLE HOADONCT(
ID INT IDENTITY PRIMARY KEY,
IDSACH INT,
IDHD INT,
DonGia decimal(10,2),
SoLuong INT,
THANHTIEN decimal(10,2)
)
-- CHAY LAN 18
CREATE FUNCTION AUTO_MAKM()
RETURNS VARCHAR(5)
AS
BEGIN
	DECLARE @ID VARCHAR(5)
	IF (SELECT COUNT(MAKM) FROM khuyenmai) = 0
		SET @ID = '0'
	ELSE
		SELECT @ID = MAX(RIGHT(MAKM, 3)) FROM khuyenmai
		SELECT @ID = CASE
			WHEN @ID >= 0 and @ID < 9 THEN 'KM' + CONVERT(CHAR, CONVERT(INT, @ID) + 1)
			WHEN @ID >= 9 THEN 'KM' + CONVERT(CHAR, CONVERT(INT, @ID) + 1)
		END
	RETURN @ID
END
-- CHAY LAN 19
IF OBJECT_ID('KHUYENMAI') IS NOT NULL
DROP TABLE KHUYENMAI
GO
CREATE TABLE KHUYENMAI
(
	ID int Identity PRIMARY KEY not null,
	MAKM VARCHAR(50) UNIQUE DEFAULT DBO.AUTO_MAKM(),
	TENKM nvarChar(100),
	CHIETKHAU INT ,
	NGAYAPDUNG DATETIME,
	NGAYKETTHUC DATETIME,
	TRANGTHAI bit DEFAULT 0
)
-- CHAY LAN 20

-- SAHCCT-NNXB
ALTER TABLE SACH ADD FOREIGN KEY (IdNXB) REFERENCES NXB(Id)
-- HoaDonBan - NguoiDung
ALTER TABLE HoaDon ADD FOREIGN KEY(IDND) REFERENCES NguoiDung(Id)
-- HoaDonBan - KhachHang
ALTER TABLE HoaDon ADD FOREIGN KEY(IdKH) REFERENCES KhachHang(Id)
-- ChiTietHoaDon - SachChiTiet
ALTER TABLE HoaDonCT ADD FOREIGN KEY(IdSach) REFERENCES SACH(Id)
-- ChiTietHoaDon - HoaDonBan
ALTER TABLE HoaDonCT ADD FOREIGN KEY(IdHD) REFERENCES HoaDon(Id)
-- HOADON-KHUYENMAI
ALTER TABLE HoaDon ADD FOREIGN KEY(IDKHUYENMAI) REFERENCES KHUYENMAI(Id)

--- phan nay de them du lieu vao db thích thì chạy, không thì thôi
select* from nguoidung 1
select* from khuyenmai 1
insert into khuyenmai(TENKM, CHIETKHAU, NGAYAPDUNG, NGAYKETTHUC, TRANGTHAI) values('Mung 8/3', 10, '2020/3/4', '2020/3/5', 0)
select* from khachhang 1
select* from hoadon
insert into hoadon(idkh, idnd, tongtien, IDKHUYENMAI, TONGTIENSAUKHUYENMAI, NgayTao, TrangThai)
values(1, 1, 30000, 1, 270000, '2020/3/4', 0)
INSERT INTO hoadon (idkh, idnd, tongtien, idkhuyenmai, tongtiensaukhuyenmai, ngaytao, trangthai)
VALUES (1, 1, 300.00, 1, 2700.00, '2020/03/04', 0);

ALTER TABLE hoadon
ALTER COLUMN tongtien DECIMAL(10, 2),
ALTER COLUMN tongtiensaukhuyenmai DECIMAL(10, 2);
 SELECT hoadon.id AS id, KHACHHANG.hoten AS ten_khach_hang, NGUOIDUNG.hoten AS ten_nguoi_dung, hoadon.ma AS ma_hoa_don, hoadon.tongtien AS tong_tien, khuyenmai.chietkhau AS giam_gia, hoadon.TONGTIENSAUKHUYENMAI AS tong_tien_khach_can_tra, hoadon.ngaytao AS ngay_tao, hoadon.trangthai AS trang_thai\n"
                    + "FROM hoadon \n"
                    + "INNER JOIN KHACHHANG ON hoadon.IDkh = KHACHHANG.id\n"
                    + "INNER JOIN nguoidung ON HOADON.idnd = NGUOIDUNG.id\n"
                    + "LEFT JOIN KHUYENMAI  ON HOADON.idkhuyenmai = KHUYENMAI.id;
					select* from sach
SELECT sach.Ma, sach.TENSACH, hoadonct.SoLuong, HOADONCT.DonGia
FROM hoadonct
JOIN sach ON hoadonct.IDSACH = sach.ID
where hoadonct.idhd = 14
select* from HOADON
insert into HOADONct(IDHD, IDSACH, SoLuong, DonGia, THANHTIEN)
values(14, 1, 2, 100000, 200000)